...
class StateTSP(NamedTuple):
    ...
    @staticmethod
    def initialize(loc, point_number, visited_dtype=torch.uint8):

        batch_size, n_loc, _ = loc.size()
        prev_a = torch.zeros(batch_size, 1, dtype=torch.long, device=loc.device)
        original_number = point_number
        selecte_number = n_loc - original_number
        return StateTSP(
            loc=loc,
            ...
            visited_=(torch.cat([ 
                 torch.ones(
                    batch_size, 1, original_number,
                    dtype=torch.uint8, device=loc.device
                ),
                torch.zeros(
                    batch_size, 1, selecte_number,
                    dtype=torch.uint8, device=loc.device
                )],2)
    ...
    def all_finished(self, point_number,maxlimt):
        # Exactly n steps
        limit = min(point_number -1 ,maxlimt - point_number)
        return self.i.item() >= limit
